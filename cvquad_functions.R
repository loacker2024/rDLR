#' sim_cv.myquad
sim_cv.myquad = function(t=NULL, x, y, intercept = TRUE
                           , plot = FALSE, type_basis = "quad",
  ...
  for (kk in 1:1){ # this for-loop can be used to tune other hyper-parameters
    ...
    for (i in seq(numFolds)) {
      ...
      possibleError = tryCatch({
        ...
        # use the function sim_myspline for the cv_myquad
        outlist_tmp = sim_myspline (x=x_sub, response = y_sub, tim = t_sub, intercept = intercept, type_basis = type_basis,
      ...
      # obtain the pred at the test x directly, without the coefficients
      # pred_test = predict( outlist_tmp, newdata = newx_sub, type = "response") # contributions from each terms in gam
      # pred_test = predict( outlist_tmp, newdata = x[which, ], type = "response") # contributions from each terms in gam
  ...
  if (is.null(metric_dev_raw_mat) ) {
    # even knots = 3 cannot solve the problem, too many coefficients
    out = list(knots_vec=NA, cvm=NA, cvsd=NA, cvup=NA, cvlo=NA, cvmin=NA, which_cvmin=NA
    ...
    colnames(metric_dev_raw_mat) = colnames(metric_misclass_raw_mat) = seq(1,knots_end) #knots_vec[1:knots_end] # cause I removed the knots parameter vector
    ...
    # knots_cvmin = knots_vec[which_cvmin]

    myspline.object = sim_myspline(x=x, response=y, intercept = intercept, type_basis = type_basis, family = family, verbose = verbose)
